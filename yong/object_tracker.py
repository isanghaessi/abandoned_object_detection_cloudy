...
def main(_argv):
    ...
    while True:
        ...
        # ------------------------------
        # yong
        # 데모 위해서 person, laptop만
        allowed_classes = ['person', 'laptop']
        # ------------------------------
        ...
        cur = collections.defaultdict(collections.defaultdict)
        # 거리 계산 함수
        ...
        # draw bbox on screen
            ...
            # Draw Circle in Bbox Center
            cv2.circle(frame, (center_x, center_y), 5, color, -1)

        # ------------------------------
            # yong
            # 사람과 사람이 아닌 물건 구별해서 보관
            if class_name == 'person':
                people[class_name + str(track.track_id)] = [center_x,
                                                            center_y, (bbox[2] + bbox[3]) / 2]
            else:
                objects[class_name + str(track.track_id)
                        ] = [center_x, center_y]
        ...
        for p in people:
            for o in objects:
                if calculateDist(people[p][:2], objects[o]) <= people[p][2] / 2:
                    pair_count[p + o] += 1
                    if pair_count[p + o] > 30:
                        if [p, o] not in pair:
                            pair.append([p, o])
                            print('pair detected: ' + p + ', ' + o)
                else:
                    if [p, o] in pair:
                        pair.remove([p, o])
                        print(p + ' and ' + o + ' was pair, and gone...')
                    pair_count[p + o] = 0
        for p in pair:
            if p[0] not in people:
                pair.remove(p)
                print(p[0] + ' and ' + p[1] + ' was pair, and gone...')
        # ------------------------------
